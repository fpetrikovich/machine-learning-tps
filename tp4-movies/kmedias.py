...
class KMeans:
    ...
    def apply(self):
        ...
        for group_index in groups:
            positivity[group_index] = {}
            for point_index in groups[group_index]:
                # Count instances per class to define the class
                _class = self.classifications[point_index]
                if not _class in positivity[group_index]:
                    positivity[group_index][_class] = 0    
                positivity[group_index][_class] += 1
            # Determine the class
            positivity[group_index] = max(positivity[group_index], key=positivity[group_index].get)
